...
class ModelManager:
    ...
    def download_model(self, model_name, progress_callback=None, quantization=None):
        ...
        logger.info(f"开始下载模型: {model_name}, 量化版本: {quantization}")
        ...
        logger.info(f"下载信息 - ModelID: {model_id}, 文件: {filename}, Ollama名称: {ollama_name}")
        ...
        if existing_path and os.path.exists(existing_path):
            logger.info(f"文件已存在: {existing_path}")
            if progress_callback:
    ...
    def _download_with_progress(self, model_id, filename, local_dir, progress_callback):
        ...
        while not result['done']:
            ...
            if current_size > 0:
                # 估算进度（假设文件大小，实际无法准确知道）
                # 这里用简单的方式：每秒检查一次，根据大小变化估算
                size_mb = current_size / (1024 * 1024)
                if progress_callback:
                    progress_callback(min(15 + int(size_mb / 10), 90), f"下载中: {size_mb:.1f} MB...")
                last_size = current_size
            elif check_count % 5 == 0 and progress_callback:
